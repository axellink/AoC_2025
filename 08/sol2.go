...
func Sol2() {
	...
	for _, n := range distances {
		...
		if i1 == -1 && i2 == -1 {
			...
			if len(circuits[len(circuits)-1].circuit) == len(positions) {
				count = n.p1.x * n.p2.x
				break
			}
		} else if i1 != -1 && i2 == -1 {
			circuits[i1].Add(n.p2)
			if len(circuits[i1].circuit) == len(positions) {
				count = n.p1.x * n.p2.x
				break
			}
		} else if i1 == -1 && i2 != -1 {
			circuits[i2].Add(n.p1)
			if len(circuits[i2].circuit) == len(positions) {
				count = n.p1.x * n.p2.x
				break
			}
		} else if i1 != -1 && i2 != -1 {
			var curr_circ Circuit
			if i1 != i2 {
				circuits[i1].Merge(circuits[i2])
				curr_circ = circuits[i1]
				circuits = slices.Delete(circuits, i2, i2+1)
			}
			if len(curr_circ.circuit) == len(positions) {
				count = n.p1.x * n.p2.x
				break
			}
		}
